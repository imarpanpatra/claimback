...
export function referenceCompensation({ regime, distanceKm, arrivalDelayMin, cancelled, intraEU }) {
  if (regime === 'EU261' || regime === 'UK261') {
    ...
    // Article 7(2)(c): the airline may halve it when the delay does not exceed 4 hours.
  ...
  if (regime === 'DGCA') {
    if (cancelled) {
      // The rules set the amount by block time, which flight data doesn't give
      // directly; distance is a close stand-in.
      const [amount, band] = distanceKm <= 500
        ? [5000, 'block time up to 1 hour (estimated from distance)']
        : distanceKm <= 1200
          ? [7500, 'block time of 1 to 2 hours (estimated from distance)']
          : [10000, 'block time over 2 hours (estimated from distance)'];
      return {
        regime,
        currency: 'INR',
        band,
        ...
        reducedAmount: 0,
        eligible: 'depends',
        note: `₹${amount.toLocaleString('en-IN')} for a flight this long, or the fare plus fuel charge if that is lower, for a cancellation with under two weeks’ notice, unless an alternative flight within two hours was offered.`,
      };
    }
    return { regime, currency: 'INR', band: 'by block time', amount: 0, reducedAmount: 0, eligible: false, note: 'India’s rules give meals, a refund or a hotel for long delays, not cash compensation.' };
  }
