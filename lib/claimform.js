...
// them, so clear them in code first. Rejecting is always the choice made.
...
async function dismissConsent(page) {
  await page.waitForSelector(CONSENT_SELECTORS.join(', '), { timeout: 4000 }).catch(() => {});
...
export async function fillClaimForm({ page, url, facts, emit, step }) {
  ...
  if (consent) {
    emit({ type: 'action', step, op: 'click', label: `${consent} (cookie banner)`, value: '' });
    // Some consent tools re-render the page after a choice is made.
    await waitForForm(page, 10_000);
  }
