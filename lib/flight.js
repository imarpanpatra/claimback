...
// The bundled table can list several active airlines under one code, such as a
// cargo arm next to the passenger airline. Passengers fly the passenger one.
function bundledAirline(iata) {
  const listed = AIRLINES[iata] ?? [];
  const active = listed.filter((a) => a.active);
  return active.find((a) => !/\bcargo\b/i.test(a.name)) ?? active[0] ?? listed[0] ?? null;
}
...
export async function resolveAirline(iata) {
  ...
  const local = bundledAirline(iata);
...
// A time zone Intl doesn't recognise would throw and end the run, so those times
// are shown in UTC instead.
const tzOf = (airport) => {
  const zone = (airport?.TZ ?? 'UTC').replace(/^:/, '');
  try {
...
const localTime = (sec, timeZone) =>
  sec
    ? `${new Intl.DateTimeFormat('en-GB', { timeZone, day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', hourCycle: 'h23' }).format(new Date(sec * 1000))}${timeZone === 'UTC' ? ' UTC' : ''}`
    : null;

const iso = (sec) => (sec ? new Date(sec * 1000).toISOString() : null);

// FlightAware sometimes keeps a cancelled placeholder next to the flight that
// actually operated on the same date.
export function pickFlight(flights, date) {
  const onDate = flights.filter((f) => f.gateDepartureTimes?.scheduled && localDate(f.gateDepartureTimes.scheduled, tzOf(f.origin)) === date);
  return onDate.find((f) => !f.cancelled) ?? onDate[0] ?? null;
}
