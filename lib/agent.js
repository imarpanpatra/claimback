...
// different from the page, so compare letters, digits and currency signs only.
const normalize = (s) =>
  String(s ?? '')
    .replace(/\]\([^)]*\)/g, ' ')
    .toLowerCase()
...
// A quote only counts as evidence if it really is on the page the agent read.
const quoteOnPage = (quote, page) => {
  const q = normalize(quote).slice(0, 120).trim();
...
Put the full amount in "amount" (0 if nothing is owed). If the rules let the airline reduce it for this flight, put the reduced figure in "reducedAmount", otherwise 0. EU and UK regulators let the airline halve compensation under Article 7(2)(c) when a flight over 3,500 km arrives between 3 and 4 hours late, so give the halved figure in that case.
...
async function claimSteps({ flightNumber, date, passenger, skipForm = false }, emit) {
  ...
  emit({ type: 'flight', step: 'flight', flight: { label, airline: airline.name, date, ...flight } });
  if (flight.cancelled) {
  ...
  const regimes = applicableRegimes({ originCountry: from.countryCode, destinationCountry: to.countryCode, airlineCountry: airline.countryCode });
  ...
  for (const regime of regimes) {
    ...
    const expected = final.eligible;
    const amountMatches = [reference.amount, reference.reducedAmount].includes(ruling.amount) && ruling.currency?.toUpperCase() === reference.currency;
    // "depends" against "yes" with the same amount is a difference in caution,
    // not a different answer.
    const agrees = expected === 'no' ? verdict === 'no' : verdict !== 'no' && amountMatches;

    let message;
    if (!agrees) {
      message = `Guardrail override: the reading gave “${verdict}, ${ruling.amount} ${ruling.currency}”, but the ${regime.id} table gives “${expected}, ${reference.amount} ${reference.currency}”. Using the table.`;
    } else if (expected === 'no') {
      message = `Guardrail passed: nothing owed under ${regime.id}, matching Claimback's built-in table.`;
    } else {
      message = `Guardrail passed: ${money(reference.amount, reference.currency)} matches Claimback's built-in ${regime.id} table.`;
      if (verdict !== expected) message += ' The reading was more cautious, since the airline could still argue extraordinary circumstances.';
    }
  ...
  if (process.env.ANAKIN_API_KEY) {
    ...
    try {
      const query = `${airline.name} ${label} ${date} ${from.city} ${to.city} delay`;
      ...
      for (const s of cause.relevantSources) {
        if (results.some((r) => r.url === s.url)) emit({ type: 'source', step: 'cause', url: s.url, title: s.title, quote: s.evidence, verified: null });
      }
    } catch (err) {
  ...
  let filing = null;
  if (!skipForm) {
    step('form', 'act', `Fill in ${airline.name}'s claim form`);
    if (!process.env.ANAKIN_API_KEY) {
      note('form', 'Skipped: finding and filling the form needs an Anakin API key for Search and the cloud browser.');
    } else {
      try {
        filing = await fileClaim({ airline, history, best, claimFacts, emit, tool, note, thought });
      } catch (err) {
  ...
  // 7. The letter, for email or for the form's free-text box
...
async function fileClaim({ airline, history, best, claimFacts, emit, tool, note, thought }) {
  const domain = hostOf(history.airline?.url);
  const query = `${airline.name} ${best.regime} flight delay compensation claim form`;
  const { results, credits } = await search(query, 8);
  ...
  const session = await openBrowser({ record: true });
  note('form', `Opened a recorded Anakin cloud browser on ${hostOf(choice.url)}.`);
  let result;
  try {
  ...
  const recording = await findRecording(session.startedAt).catch(() => null);
