...
export async function askJson({ system, prompt, schema, images = [], timeoutMs = 120_000 }) {
  ...
  for (let attempt = 1; ; attempt++) {
    ...
    try {
      ...
      if (attempt >= 3) throw err.name === 'TimeoutError' ? new Error(`OpenAI timed out after ${timeoutMs / 1000}s`) : err;
