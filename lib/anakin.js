...
// Job statuses that mean the work ended without a result.
...
// Any status other than completed or a known failure counts as still running,
// and a timeout names the last one seen.
export async function scrape(url, { timeoutMs = 150_000 } = {}) {
  let job = await call('POST', '/url-scraper/scrape', { url, useBrowser: false });
  const deadline = Date.now() + timeoutMs;
  while (job.status !== 'completed') {
    if (FAILED.has(job.status)) {
      throw new Error(`Scraping ${url} ${job.status === 'failed' ? 'failed' : `ended as ${job.status}`}: ${job.error ?? 'unknown error'}`);
    }
    if (Date.now() > deadline) throw new Error(`Scraping ${url} timed out (last status: ${job.status ?? 'none'})`);
...
export async function openBrowser({ record = true, country } = {}) {
  ...
  try {
    ...
    // A connected session is billed until it closes, so never leave one open.
...
export async function findRecording(startedAt, timeoutMs = 60_000, { endedAt = Date.now() } = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    if (FAILED.has(mine?.status)) return null;
    await sleep(4000);
  }
  return null;
}
